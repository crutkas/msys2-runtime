...
static void
write_process_instruction (CONTEXT_REG address, const unsigned char *insn)
{
  SIZE_T written;

  if (!WriteProcessMemory (hProcess, (void *) address, insn,
			   SW_BREAKPOINT_SIZE, &written)
...
#error unimplemented for this target
  ...
  if (!ReadProcessMemory (hProcess, (void *) address,
