...
public static class Mile {
  ...
  static int vB3, vB4; static long vV3, vV4;
  static bool SetBps(int tid, IntPtr ctx, long probe, long advance) {
    ...
    if (ok && fetchPc != 0 && !verified) {
      ...
      if (GetThreadContext(th, ctx)) {
        ...
        vB3 = Marshal.ReadInt32(ctx, 0x324); vV3 = Marshal.ReadInt64(ctx, 0x350);
        vB4 = Marshal.ReadInt32(ctx, 0x328); vV4 = Marshal.ReadInt64(ctx, 0x358);
  ...
  public static string Run(string exe, string cwd) {
    ...
    while (WaitForDebugEvent(ev, 30000)) {
      ...
      if (code == 6 || code == 2 || code == 3) {
        ...
        if (armed && ec == 0x80000003) {
          ...
          if (fetchPc != 0 && !advancing.Contains(tid) && (pc == fetchPc || pc == fetchPc-0xB574CL+0x1122C8L || pc == fetchPc-0xB574CL+0xB4EB8L)) {
          ...
          if (advancing.Contains(tid)) {
            ...
            if (x5 == 0x1L) { sampling = true; lastChain = 0x8000068C0L;
    ...
    sb.AppendLine("READBACK Bcr[3]=0x" + vB3.ToString("X") + " Bvr[3]=0x" + vV3.ToString("X")
                  + "   Bcr[4]=0x" + vB4.ToString("X") + " Bvr[4]=0x" + vV4.ToString("X"));
    sb.AppendLine("Bcr[3]/Bcr[4] HONOURED? " + ((vB3 != 0 && vB4 != 0) ? "YES" : "NO - platform kept fewer breakpoint registers"));
